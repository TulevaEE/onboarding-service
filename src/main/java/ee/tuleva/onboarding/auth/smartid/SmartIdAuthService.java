    ...
    poller.submit(
        () -> {
          try {
            ...
            log.info(
                "Smart ID timed out waiting for the user: personalCode={}",
                session.getPersonalCode());
            session.setErrorCode("smart.id.timeout");
            session.setErrorMessage("Smart ID timed out waiting for the user");
          } catch (Exception e) {
