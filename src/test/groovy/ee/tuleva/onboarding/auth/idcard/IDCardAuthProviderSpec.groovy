...
class IDCardAuthProviderSpec extends Specification {
  ...
  def "supports id card"() {
    expect:
    idCardAuthProvider.supports(ID_CARD)
    !idCardAuthProvider.supports(MOBILE_ID)
    !idCardAuthProvider.supports(SMART_ID)
  }
  ...
  def "returns person when login is complete"() {
    ...
    principalService.getFrom(session, [
        (ID_DOCUMENT_TYPE): DIGITAL_ID_CARD.name(),
        (GRANT_TYPE)      : ID_CARD.name()
    ]) >> person
    when:
