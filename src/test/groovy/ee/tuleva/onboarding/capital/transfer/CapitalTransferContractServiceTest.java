...
class CapitalTransferContractServiceTest {
  ...
  @Mock private CapitalTransferCreationValidator validator;
  ...
  void create() {
    ...
    when(memberService.getById(sampleCommand.getBuyerMemberId())).thenReturn(buyer);
    ...
    verify(validator).validate(sellerUser.getMemberOrThrow(), buyer, sampleCommand);
  }

  @Test
  @DisplayName("Create capital transfer propagates validation failure")
  void createPropagatesValidationFailure() {
    ...
    when(userService.getById(sellerPerson.getUserId())).thenReturn(Optional.of(sellerUser));
    when(memberService.getById(sampleCommand.getBuyerMemberId())).thenReturn(buyer);
    doThrow(new IllegalStateException("Seller does not have enough member capital"))
        .when(validator)
        .validate(sellerUser.getMemberOrThrow(), buyer, sampleCommand);
    ...
    verifyNoInteractions(contractContentService);
  }
      ...
      if (state == BUYER_SIGNED) {
        ...
        verify(emailSender)
            .sendContractEmail(
                ...
                eq(CAPITAL_TRANSFER_CONFIRMED_BY_BUYER),
                eq(contract));
      ...
      if (state == PAYMENT_CONFIRMED_BY_BUYER) {
        ...
        verify(emailSender)
            .sendContractEmail(
                ...
                eq(CAPITAL_TRANSFER_CONFIRMED_BY_SELLER),
                eq(contract));
  ...
  void signBySeller() {
    ...
    verify(emailSender)
        .sendContractEmail(
            eq(contract.getBuyer().getUser()), eq(CAPITAL_TRANSFER_BUYER_TO_SIGN), eq(contract));
  }
