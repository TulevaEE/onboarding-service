...
class KybScreeningServiceTest {
  ...
  void singlePersonCompanyRunsRules31And34() {
    var person = new KybRelatedPerson("38501010001", true, true, true, BigDecimal.valueOf(100));
    ...
    assertThat(types).containsExactlyInAnyOrder(SOLE_MEMBER_OWNERSHIP, COMPANY_ACTIVE);
    assertThat(results).allMatch(KybCheck::success);
  }

  @Test
  void twoPersonCompanyWithTwoBoardMembersRunsRules32And34() {
    var person1 = new KybRelatedPerson("38501010001", true, true, true, BigDecimal.valueOf(50));
    var person2 = new KybRelatedPerson("38501010002", true, true, true, BigDecimal.valueOf(50));
    var data = new KybCompanyData("12345678", "38501010001", R, List.of(person1, person2));

    var results = kybScreeningService.screen(data);

    var types = results.stream().map(KybCheck::type).toList();
    assertThat(types).containsExactlyInAnyOrder(DUAL_MEMBER_OWNERSHIP, COMPANY_ACTIVE);
    assertThat(results).allMatch(KybCheck::success);
  }
  ...
  void twoPersonCompanyWithOneBoardMemberRunsRules33And34() {
    var person1 = new KybRelatedPerson("38501010001", true, true, true, BigDecimal.valueOf(50));
    var person2 = new KybRelatedPerson("38501010002", false, true, true, BigDecimal.valueOf(50));
    var data = new KybCompanyData("12345678", "38501010001", R, List.of(person1, person2));

    var results = kybScreeningService.screen(data);

    var types = results.stream().map(KybCheck::type).toList();
    assertThat(types).containsExactlyInAnyOrder(SOLE_BOARD_MEMBER_IS_OWNER, COMPANY_ACTIVE);
    assertThat(results).allMatch(KybCheck::success);
  }
