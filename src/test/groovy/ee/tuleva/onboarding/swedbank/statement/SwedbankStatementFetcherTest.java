...
class SwedbankStatementFetcherTest {
  ...
  void setup() {
    ...
    fetcher = new SwedbankStatementFetcher(TestClockHolder.clock, fetchJobRepository, swedbankGatewayClient);
  }
  ...
  void testSendRequest() {
    ...
    var mockWaitingForReplyJob = SwedbankStatementFetchJob.builder().jobStatus(WAITING_FOR_REPLY).id(id).build();

    when(fetchJobRepository.findFirstByJobStatusOrderByCreatedAtDesc(any())).thenReturn(Optional.empty());
    when(fetchJobRepository.save(any())).thenReturn(mockScheduledJob).thenReturn(mockWaitingForReplyJob);

    when(swedbankGatewayClient.getAccountStatementRequestEntity(any(), any())).thenReturn(mockDocument);

    Set<JobStatus> savedStatuses = EnumSet.noneOf(JobStatus.class);

    when(fetchJobRepository.save(any())).thenAnswer(invocation -> {
  ...
  void testSendRequestLastOneMoreThanOneHourAgo() {
    ...
    var lastScheduledJob = SwedbankStatementFetchJob.builder().jobStatus(SCHEDULED).createdAt(TestClockHolder.now.minus(65, MINUTES)).id(UUID.randomUUID()).build();

    var mockScheduledJob = SwedbankStatementFetchJob.builder().jobStatus(SCHEDULED).id(id).build();
    var mockWaitingForReplyJob = SwedbankStatementFetchJob.builder().jobStatus(WAITING_FOR_REPLY).id(id).build();

    when(fetchJobRepository.findFirstByJobStatusOrderByCreatedAtDesc(any())).thenReturn(Optional.of(lastScheduledJob));
    when(fetchJobRepository.save(any())).thenReturn(mockScheduledJob).thenReturn(mockWaitingForReplyJob);

    when(swedbankGatewayClient.getAccountStatementRequestEntity(anyString(), eq(id))).thenReturn(mockDocument);

    fetcher.sendRequest();

    verify(fetchJobRepository, times(2)).save(argThat(job -> job.getJobStatus().equals(WAITING_FOR_REPLY) || job.getJobStatus().equals(SCHEDULED)));
    verify(swedbankGatewayClient, times(1)).sendStatementRequest(mockDocument, id);
  }


  @Test
  @DisplayName("request sender does not create multiple jobs when last one was less than 1 hour ago")
  void testCreateJobLessThanOneHourAgo() {
    var lastScheduledJob = SwedbankStatementFetchJob.builder().jobStatus(SCHEDULED).createdAt(TestClockHolder.now).id(UUID.randomUUID()).build();
  ...
  void getAndSaveResponse() {
    ...
    var mockWaitingForReplyJob = SwedbankStatementFetchJob.builder().jobStatus(WAITING_FOR_REPLY).id(id).createdAt(TestClockHolder.now.minus(2, MINUTES)).build();
    when(fetchJobRepository.findFirstByJobStatusOrderByCreatedAtDesc(eq(WAITING_FOR_REPLY))).thenReturn(Optional.of(mockWaitingForReplyJob));

    var mockSwedbankResponse = new SwedbankGatewayResponseDto("<xml>TEST</xml>", id, trackingId.toString());
  ...
  void doNothingJobNotAvailable() {
    ...
    when(fetchJobRepository.findFirstByJobStatusOrderByCreatedAtDesc(eq(WAITING_FOR_REPLY))).thenReturn(Optional.empty());
  ...
  void doNothingWhenLessThanOneMinutePassedFromLastCheck() {
    ...
    var mockWaitingForReplyJob = SwedbankStatementFetchJob.builder().jobStatus(WAITING_FOR_REPLY).id(id).createdAt(TestClockHolder.now.minus(30, SECONDS)).build();
    when(fetchJobRepository.findFirstByJobStatusOrderByCreatedAtDesc(eq(WAITING_FOR_REPLY))).thenReturn(Optional.of(mockWaitingForReplyJob));
  ...
  void responseNotReady() {
    ...
    var mockWaitingForReplyJob = SwedbankStatementFetchJob.builder().jobStatus(WAITING_FOR_REPLY).id(id).createdAt(TestClockHolder.now.minus(5, MINUTES)).lastCheckAt(TestClockHolder.now.minus(2, MINUTES)).build();
    when(fetchJobRepository.findFirstByJobStatusOrderByCreatedAtDesc(eq(WAITING_FOR_REPLY))).thenReturn(Optional.of(mockWaitingForReplyJob));
    ...
    verify(fetchJobRepository, times(1)).save(argThat(job -> job.getJobStatus() == WAITING_FOR_REPLY && job.getLastCheckAt() == TestClockHolder.now));
  }
  ...
  void throwInvalidResponse() {
    ...
    var mockSwedbankResponse = new SwedbankGatewayResponseDto("<xml>TEST</xml>", swedbankBrokenId, trackingId.toString());
    when(swedbankGatewayClient.getResponse()).thenReturn(Optional.of(mockSwedbankResponse));
  ...
  void getResponseFailedMarshal() {
    ...
    when(swedbankGatewayClient.getParsedStatementResponse(any())).thenThrow(new IllegalStateException("Broken XML"));
