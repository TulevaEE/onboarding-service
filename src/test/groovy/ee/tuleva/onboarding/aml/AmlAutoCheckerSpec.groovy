...
class AmlAutoCheckerSpec extends Specification {
  ...
  def "checks user before login"() {
    ...
    AuthenticationAttributes attributes = new AuthenticationAttributes()
    attributes.setDocumentType(ESTONIAN_CITIZEN_ID_CARD);

    when:
    amlAutoChecker.beforeLogin(new BeforeTokenGrantedEvent(this, person, attributes, GrantType.ID_CARD))
  ...
  def "throws exception when user not found"() {
    ...
    when:
    amlAutoChecker.beforeLogin(new BeforeTokenGrantedEvent(this, person, new AuthenticationAttributes(), GrantType.MOBILE_ID))
  ...
  def "adds pension registry name check if missing on login"() {
    ...
    when:
    amlAutoChecker.afterLogin(new AfterTokenGrantedEvent(this, user, new AuthenticationAttributes(), GrantType.ID_CARD))
