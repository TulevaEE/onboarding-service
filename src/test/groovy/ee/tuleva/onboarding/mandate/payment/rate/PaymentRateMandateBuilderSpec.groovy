...
class PaymentRateMandateBuilderSpec extends Specification {
  ...
  def "build creates mandate with given payment rate"() {
    ...
    when:
    Mandate mandate = paymentRateMandateBuilder.build(paymentRate, user, conversion, contactDetails)

    then:
    mandate.paymentRate == paymentRate
    mandate.user == user
    mandate.address == contactDetails.address
    1 * conversionDecorator.addConversionMetadata(_, conversion, contactDetails)
  }
}
