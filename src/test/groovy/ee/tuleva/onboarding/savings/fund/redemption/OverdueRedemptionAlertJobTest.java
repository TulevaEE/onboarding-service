...
class OverdueRedemptionAlertJobTest {
    ...
    verify(notificationService)
        .sendMessage(
            "AML: 1 redemption request(s) waiting past their deadline:\n"
                + "id=98894b5d-0326-45f6-80a7-994086571773, status=IN_REVIEW,"
                + " reason=SCREENING_UNAVAILABLE, amount=10.00 EUR,"
  ...
  @Test
  void aReservedRequestStuckInVerificationIsReportedLikeAHeldOne() {
    given(repository.findByStatusIn(OPEN_STATUSES))
    ...
    verify(notificationService)
        .sendMessage(
            "AML: 1 redemption request(s) waiting past their deadline:\n"
                + "id=98894b5d-0326-45f6-80a7-994086571773, status=RESERVED,"
                + " reason=null, amount=10.00 EUR,"
                + " requested=2026-08-27 22:19, decisionCutoff=2026-08-28 16:00,"
  ...
  void aVerifiedRequestTheBatchDidNotPayIsOverdueTheNextMorning() {
    ...
    verify(notificationService)
        .sendMessage(
            "AML: 1 redemption request(s) waiting past their deadline:\n"
                + "id=98894b5d-0326-45f6-80a7-994086571773, status=VERIFIED,"
                + " reason=null, amount=10.00 EUR,"
                + " requested=2026-08-27 22:19, decisionCutoff=2026-08-28 16:00,"
  ...
  @Test
  void aFailedPayoutIsAlwaysOverdue() {
    given(repository.findByStatusIn(OPEN_STATUSES))
    ...
    verify(notificationService)
        .sendMessage(
            "AML: 1 redemption request(s) waiting past their deadline:\n"
                + "id=98894b5d-0326-45f6-80a7-994086571773, status=FAILED,"
                + " reason=null, amount=10.00 EUR,"
                + " requested=2026-08-27 22:19, decisionCutoff=2026-08-28 16:00,"
                + " workingDaysWaiting=1",
            AML,
