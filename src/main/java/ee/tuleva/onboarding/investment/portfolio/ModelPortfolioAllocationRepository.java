...
public interface ModelPortfolioAllocationRepository
  ...
  @Query("SELECT DISTINCT m.fund FROM ModelPortfolioAllocation m WHERE m.isin = :isin")
  List<TulevaFund> findFundsWhoseModelNamed(String isin);

  @Query(
      """
      SELECT MIN(m.effectiveDate) FROM ModelPortfolioAllocation m
      WHERE m.fund = :fund
      AND m.effectiveDate > (
        SELECT MAX(named.effectiveDate)
        FROM ModelPortfolioAllocation named
        WHERE named.fund = :fund AND named.isin = :isin
      )
      """)
  Optional<LocalDate> findEffectiveDateOfFirstVersionWithout(TulevaFund fund, String isin);
