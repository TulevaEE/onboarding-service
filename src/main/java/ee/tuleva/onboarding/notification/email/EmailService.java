...
public class EmailService {
  ...
  public void sendMandate(User user, Long mandateId, byte[] file) {
    ...
    if (mandrillApi == null) {
      log.warn(
          "Mandrill not initialised, not sending mandate email for user {} and mandate {}.",
          user.getId(),
          mandateId);
      return;
    }
    ...
    MandrillMessage message =
        newMandrillMessage(
            ...
            emailContentService.getMandateEmailHtml(),
            getMandateTags(),
  ...
  public void sendMemberNumber(User user) {
    ...
    if (mandrillApi == null) {
      log.warn(
          "Mandrill not initialised, not sending member number email for userId {}, member #",
          user.getId(),
  ...
  private MandrillMessage newMandrillMessage(
      List<Recipient> to,
      String subject,
      String html,
      List<String> tags,
