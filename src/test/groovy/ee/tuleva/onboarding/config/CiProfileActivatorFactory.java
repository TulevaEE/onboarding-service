 ...
 * the "pg" profile is active (SPRING_PROFILES_ACTIVE=pg,test).
 *
 * <p>This factory is needed because {@link TestcontainersConfiguration} uses
 * {@code @TestConfiguration}, which is not component-scanned. The {@code @Profile("pg")} on
 * TestcontainersConfiguration ensures the PostgreSQL container is only created when requested.
 */
public class CiProfileActivatorFactory implements ContextCustomizerFactory {
  ...
  private static class CiProfileActivator implements ContextCustomizer {
    ...
    public void customizeContext(
      ...
      // The @Profile("pg") on the configuration ensures it only activates with Testcontainers
