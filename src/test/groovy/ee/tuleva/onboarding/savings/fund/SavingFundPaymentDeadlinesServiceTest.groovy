...
class SavingFundPaymentDeadlinesServiceTest extends Specification {
  ...
  @Unroll
  def "getFulfillmentDeadline when payment created on working day #paymentTime should return deadline #expectedDeadline"() {
    given:
    def paymentDate = LocalDate.of(2025, 9, 30) // Tuesday
    def paymentCreatedAt = ZonedDateTime.of(paymentDate, paymentTime, timeZone).toInstant()
    def payment = SavingFundPayment.builder()
    ...
    when:
    def result = service.getFulfillmentDeadline(payment)

    then:
    def expectedInstant = ZonedDateTime.of(expectedDate, LocalTime.of(16, 0), timeZone).toInstant()
    result == expectedInstant

    where:
    paymentTime                || expectedDate                           || expectedDeadline
    LocalTime.of(9, 0)        || LocalDate.of(2025, 10, 1)            || "next working day at 16:00"
    LocalTime.of(15, 58)      || LocalDate.of(2025, 10, 1)            || "next working day at 16:00"
    LocalTime.of(15, 59)      || LocalDate.of(2025, 10, 1)            || "next working day at 16:00"
    LocalTime.of(16, 0)       || LocalDate.of(2025, 10, 1)            || "next working day at 16:00"
    LocalTime.of(23, 59)      || LocalDate.of(2025, 10, 1)            || "next working day at 16:00"
  }

  def "getFulfillmentDeadline when payment created on weekend should return next working day"() {
    given:
    def saturdayDate = LocalDate.of(2025, 10, 4) // Saturday
    def paymentCreatedAt = ZonedDateTime.of(saturdayDate, LocalTime.of(10, 0), timeZone).toInstant()
    def payment = SavingFundPayment.builder()
    ...
    then:
    def expectedInstant = ZonedDateTime.of(LocalDate.of(2025, 10, 6), LocalTime.of(16, 0), timeZone).toInstant()
    result == expectedInstant
  }

  def "getFulfillmentDeadline when payment created on holiday should return next working day"() {
    given:
    def holidayDate = LocalDate.of(2025, 12, 25) // Christmas Day (assumed holiday)
    def paymentCreatedAt = ZonedDateTime.of(holidayDate, LocalTime.of(10, 0), timeZone).toInstant()
    def payment = SavingFundPayment.builder()
    ...
    then:
    def expectedInstant = ZonedDateTime.of(LocalDate.of(2025, 12, 26), LocalTime.of(16, 0), timeZone).toInstant()
    result == expectedInstant
  }

  def "getFulfillmentDeadline when payment created on Friday should return next Monday"() {
    given:
    def fridayDate = LocalDate.of(2025, 10, 3) // Friday
    def paymentCreatedAt = ZonedDateTime.of(fridayDate, LocalTime.of(16, 30), timeZone).toInstant()
    def payment = SavingFundPayment.builder()
  ...
  def "getFulfillmentDeadline handles timezone conversion correctly"() {
    given:
    def utcZone = ZoneId.of("UTC")
    def utcClock = Mock(Clock)
    utcClock.getZone() >> utcZone
    ...
    and: "Next working day setup"
    publicHolidays.nextWorkingDay(paymentDate) >> paymentDate.plusDays(1)

    when:
    def result = utcService.getFulfillmentDeadline(payment)

    then:
    def expectedInstant = ZonedDateTime.of(paymentDate.plusDays(1), LocalTime.of(16, 0), utcZone).toInstant()
    result == expectedInstant
  }

  def "getFulfillmentDeadline when working day spans holiday weekend"() {
    given:
    def thursdayDate = LocalDate.of(2025, 4, 17) // Thursday before Easter
    def paymentCreatedAt = ZonedDateTime.of(thursdayDate, LocalTime.of(16, 0), timeZone).toInstant()
    def payment = SavingFundPayment.builder()
    ...
    then:
    def expectedInstant = ZonedDateTime.of(LocalDate.of(2025, 4, 22), LocalTime.of(16, 0), timeZone).toInstant()
    result == expectedInstant
  }
}
