...
- **Strict version ordering everywhere — never enable `out-of-order`.** A migration numbered below an already-deployed version fails validation and blocks all deploys (ECS rolls back silently while CI stays green). Before merging, renumber your migrations above the current master max. A racing PR can still take your numbers after that (this happened with V1_198–V1_202: the PR that renumbered *to* V1_202 deployed first and stranded V1_198–V1_201); the merge queue rejects your PR when it does, so renumber and re-queue then rather than re-checking after every merge to master.
- **`.githooks/migration-order` enforces this** at `pre-push` and in the CircleCI `migration-order` job. CI is the load-bearing one: the race happens *after* you push, when a PR that merges first takes your numbers, and only a run against master as it stands at merge time can see it. The merge queue is that run: `master` requires `ci/circleci: migration-order`, and the queue runs it on `gh-readonly-queue/…` (master, the PRs queued ahead of yours, and yours), so two queued PRs claiming one version are caught too; a merge that bypasses the queue is checked by nothing, since on master itself the job has nothing to compare against. Only that job runs on the queue branch; the full build runs on master after the merge. Because the verdict belongs to the queue, `pre-push` warns rather than blocks: git's single `--no-verify` covers the whole hook, so a numbering complaint that stopped a push would also be a reason to switch off the `pii-scan` standing beside it, which is the one check there that must block. It rejects a version at or below master's max and a version claimed twice, reading `src/main/resources/db/migration` and `src/main/java/db/migration` as the one sequence Flyway applies them as — listing only the SQL directory is how `V1_275` came to be held by a migration in each. A gap above master's max only warns: Flyway applies `1.278` and `1.280` in order whether or not `1.279` exists, and failing on it would stop two open PRs from holding distinct numbers, which is the normal case. H2 compat migrations are compared on the full version, so `V1_270_1` beside master's `V1_270` is accepted. `.githooks/migration-order-test` self-tests the checker.
