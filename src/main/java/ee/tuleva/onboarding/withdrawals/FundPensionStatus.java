...
public record FundPensionStatus(
    List<FundPension> fundPensions) {
  public static FundPensionStatus from(FundPensionStatusDto statusDto) {
    var secondPillarFunds = statusDto.secondPillarFundPensions().stream().map(dto -> FundPension.from(dto, SECOND));
    var thirdPillarFunds = statusDto.thirdPillarFundPensions().stream().map(dto -> FundPension.from(dto, THIRD));
  ...
  public record FundPension(
    ...
    public static FundPension from(FundPensionDto fundPensionDto, Pillar pillar) {
      return new FundPension(pillar, fundPensionDto.startDate(), fundPensionDto.endDate(), fundPensionDto.durationYears(), fundPensionDto.active());
    }
  }
}
