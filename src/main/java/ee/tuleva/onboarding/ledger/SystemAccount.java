...
public enum SystemAccount {
  ...
  SECURITIES_UNITS_EQUITY(
      LIABILITY, FUND_UNIT, NAV_CALCULATION, "Balancing account for securities unit holdings"),
  ...
  TRADE_CASH_SETTLEMENT(ASSET, EUR, BANK_RECONCILIATION, "Trade cash settlements"),
  TRADE_UNIT_SETTLEMENT(ASSET, FUND_UNIT, BANK_RECONCILIATION, "Trade unit settlements"),
  SECURITIES_CUSTODY(
      LIABILITY, FUND_UNIT, BANK_RECONCILIATION, "Equity account for balancing unit settlements");
  ...
  public String getAccountName(TulevaFund fund) {
    return name() + ":" + fund.name();
  }

  public String getAccountName(TulevaFund fund, String instrument) {
