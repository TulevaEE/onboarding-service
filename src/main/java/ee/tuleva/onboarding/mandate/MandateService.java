...
public class MandateService {
  ...
  private final MobileIdSignatureService mobileIdSignatureService;
  ...
  public Mandate save(Long userId, CreateMandateCommand createMandateCommand) {
    ...
    Mandate mandate =
        mandateConverter.convert(new CreateMandateCommandWithUser(createMandateCommand, user));
  ...
  public MobileIdSignatureSession mobileIdSign(Long mandateId, Long userId, String phoneNumber) {
    ...
    return mobileIdSignatureService.startSign(files, user.getPersonalCode(), phoneNumber);
  }
  ...
  public String finalizeSmartIdSignature(
      Long userId, Long mandateId, SmartIdSignatureSession session) {
    User user = userService.getById(userId);
  ...
  public String finalizeMobileIdSignature(
      Long userId, Long mandateId, MobileIdSignatureSession session) {
    User user = userService.getById(userId);
    Mandate mandate = mandateRepository.findByIdAndUserId(mandateId, userId);
  ...
  private String handleUnsignedMandateMobileId(
    ...
    return getStatus(user, mandate, mobileIdSignatureService.getSignedFile(session));
  }

  public String finalizeIdCardSignature(
      Long userId, Long mandateId, IdCardSignatureSession session, String signedHash) {
    User user = userService.getById(userId);
    Mandate mandate = mandateRepository.findByIdAndUserId(mandateId, userId);
              ...
              .orElseThrow(
                  () ->
                      new RuntimeException(
                          "Expecting mandate to be signed, but can not access signed file.")));
  ...
  private String handleUnsignedMandateIdCard(
      User user, Mandate mandate, IdCardSignatureSession session, String signedHash) {
