...
public class TransferCancellationMandateDetails extends MandateDetails {
  ...
  public static TransferCancellationMandateDetails fromFundTransferExchanges(List<FundTransferExchange> fundTransferExchanges) {
    if (fundTransferExchanges != null
      ...
      return new TransferCancellationMandateDetails(fundTransferExchanges.getFirst().getSourceFundIsin());
    } else {
      throw new IllegalArgumentException("Cannot construct TransferCancellationMandateDetails from invalid fundTransferExchanges");
    }
  }
}
