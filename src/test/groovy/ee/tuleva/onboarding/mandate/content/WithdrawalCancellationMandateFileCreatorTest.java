...
public class WithdrawalCancellationMandateFileCreatorTest {

  @Mock private MandateContentService mandateContentService;

  @InjectMocks
  private WithdrawalCancellationMandateFileCreator withdrawalCancellationMandateFileCreator;

  @Test
  @DisplayName("creates withdrawal cancellation mandate file")
  void test_delegatesToMandateContentCreator() {
    ...
    when(mandateContentService.getMandateCancellationHtml(
            aUser, aMandate, aContactDetails, WITHDRAWAL))
        .thenReturn("withdrawalCancellationContent");

    List<MandateContentFile> files =
        withdrawalCancellationMandateFileCreator.getContentFiles(aUser, aMandate, aContactDetails);
