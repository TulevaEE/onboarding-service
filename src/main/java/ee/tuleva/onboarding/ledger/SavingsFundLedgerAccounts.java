...
class SavingsFundLedgerAccounts {
  ...
  BigDecimal holding(LedgerAccount account) {
    return inAccountPrecision(ledgerAccountService.balanceOf(account), account);
  }
  ...
  private static BigDecimal inAccountPrecision(BigDecimal balance, LedgerAccount account) {
    return balance.negate().setScale(account.getAssetType().getMaxPrecision(), HALF_UP);
  }
