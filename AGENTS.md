...
- `scripts/pitest-slice.sh <module>` — runs pitest for one top-level module and records it in `metrics/pitest-slices.json` (per-module mutations/detected/score/date). Mutation scores decompose by class, so the mutation-weighted aggregate over complete slices equals a full run; the scorecard reports it as `mutationScoreSliced`/`mutationsSliced`/`pitestSlicedModules`. The loop runs one slice per iteration as a rotation; once every module has a slice, the aggregate replaces the stale full-run `mutationScore`.
