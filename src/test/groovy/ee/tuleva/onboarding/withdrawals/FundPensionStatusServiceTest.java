...
class FundPensionStatusServiceTest {
  ...
  @InjectMocks
  private FundPensionStatusService fundPensionStatusService;
  ...
  void shouldGetFundPensionOver55() {
    ...
    var secondPillarFundPensions =
        List.of(new FundPensionStatusDto.FundPensionDto(Instant.parse("2019-10-01T12:13:27.141Z"), null, 20, true));
    ...
    assertThat(result.fundPensions().stream().filter(fundPension -> fundPension.pillar() == SECOND).count()).isEqualTo(1);
    assertThat(result.fundPensions().stream().filter(fundPension -> fundPension.pillar() == THIRD).count()).isEqualTo(1);
  }
