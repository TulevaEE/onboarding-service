...
public class InputErrorsConverter implements Converter<Errors, ErrorsResponse> {
  ...
  public ErrorsResponse convert(Errors source) {
    List<ErrorResponse> globalErrors =
        source.getGlobalErrors().stream().map(this::convert).collect(toList());
    List<ErrorResponse> fieldErrors =
        source.getFieldErrors().stream().map(this::convert).collect(toList());
  ...
  private ErrorResponse convert(FieldError error) {
    return genericConvert(error).path(error.getField()).build();
  }
