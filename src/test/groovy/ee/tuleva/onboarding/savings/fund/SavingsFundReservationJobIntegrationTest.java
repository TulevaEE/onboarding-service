...
class SavingsFundReservationJobIntegrationTest {
  ...
  void processesPaymentBeforeCutoff() {
    ...
    repository.attachParty(paymentId, new PartyId(PERSON, user.getPersonalCode()));
  ...
  void continuesProcessingWhenOnePaymentFails() {
    ...
    repository.attachParty(invalidPaymentId, new PartyId(PERSON, user.getPersonalCode()));
    ...
    var validPaymentId =
        repository.savePaymentData(createPayment().receivedBefore(beforeCutoff).build());
    repository.attachParty(validPaymentId, new PartyId(PERSON, user.getPersonalCode()));
