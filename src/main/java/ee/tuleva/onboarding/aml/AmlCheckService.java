...
public class AmlCheckService {
  ...
  public List<AmlCheckType> getMissingChecks(Long userId) {
    User user = userService.getById(userId);
    val initial = stream(AmlCheckType.values()).filter(AmlCheckType::isManual).collect(toList());
    val existing = amlService.getChecks(user).stream().map(AmlCheck::getType).collect(toList());
